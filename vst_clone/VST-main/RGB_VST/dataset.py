...
def load_list(dataset_name, data_root):
    ...
    img_root = data_root + 'sub_matching_data\\' + dataset_name
    img_files = os.listdir(img_root)

    for img in img_files:

        images.append(img_root + img[:-4]+'.png')
        labels.append(img_root.replace('sub_matching_data\\', 'sub_binary_masks\\') + img[:-4]+'.png')
        contours.append(img_root.replace('sub_matching_data\\', 'sub_boundaries\\') + img[:-4] + '.png')
...
def load_test_list(test_path, data_root):
    ...
    img_root = data_root + 'sub_matching_data\\' + test_path
